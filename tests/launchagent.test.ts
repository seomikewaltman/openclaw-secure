...
import { findPlist, readPlist, backupPlist } from '../src/launchagent.js';
...
<dict>
    ...
    <array>
        ...
        <string>/usr/local/lib/node_modules/clawdbot/dist/entry.js</string>
...
describe('findPlist', () => {
  it('throws when plist does not exist', () => {
    ...
    try {
      findPlist();
      // If it succeeds (macOS with clawdbot installed), that's fine too
    } catch (err) {
      expect(err).toBeInstanceOf(Error);
      expect((err as Error).message).toContain('Clawdbot LaunchAgent not found');
    }
  });
});
...
describe('readPlist', () => {
  ...
  it('parses a full plist with all fields', async () => {
    ...
    expect(config.label).toBe('com.clawdbot.gateway');
    expect(config.programArguments).toEqual([
      '/usr/local/bin/node',
      '/usr/local/lib/node_modules/clawdbot/dist/entry.js',
      'gateway',
      '--port',
      '18789',
    ]);
